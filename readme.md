...
**🚀 智能热点雷达系统 —— 持续扫描全网热点，精准推送你关心的资讯**

*TrendRadaring = Trend + Radar + ing，寓意持续进行的趋势雷达扫描*
...
**💡 提示：** 想要网页版？进入你的仓库 Settings → Pages，启用 GitHub Pages。
...
### 2025/08/16 - v2.0.4 (TrendRadaring)

**重大更新**：
- 🎯 **项目重命名**: TrendRadar → TrendRadaring (持续进行的趋势雷达)
- 🆕 **智能去重功能**: 自动识别和去除相似的新闻标题，避免重复信息干扰
- 📊 **相似度算法**: 基于LCS算法计算标题相似度，保留排名最高的重要新闻
- ⚙️ **可配置阈值**: 支持自定义相似度阈值，灵活控制去重严格程度
- 📱 **状态显示**: 飞书消息中显示去重状态，便于监控去重效果
...
3. **配置关键词和设置**:
    ...
    - **智能去重配置**: 在 `config/config.yaml` 中配置去重功能：
      ```yaml
      deduplication:
        enable_smart_dedup: true # 启用智能去重
