...
def load_config():
    ...
    config = {
        ...
        "DEDUP_CONFIG": {
            "ENABLE_SMART_DEDUP": config_data.get("deduplication", {}).get("enable_smart_dedup", True),
            "SIMILARITY_THRESHOLD": config_data.get("deduplication", {}).get("similarity_threshold", 0.8),
        },
...
print("正在加载配置...")
CONFIG = load_config()
print(f"TrendRadaring v{VERSION} 配置加载完成")
...
def normalize_title_for_dedup(title: str) -> str:
    ...
    # 移除常见的新闻前缀后缀
    prefixes = ['最新', '突发', '快讯', '独家', '重磅', '热点', '焦点', '关注']
    suffixes = ['最新消息', '最新进展', '详情', '全文', '视频', '图片', '现场']
...
def calculate_title_similarity(title1: str, title2: str) -> float:
    """计算两个标题的相似度"""
    norm1 = normalize_title_for_dedup(title1)
    norm2 = normalize_title_for_dedup(title2)
...
def deduplicate_similar_titles(titles_data: Dict, similarity_threshold: float = None) -> Dict:
    ...
    # 检查是否启用智能去重
    if not CONFIG.get("DEDUP_CONFIG", {}).get("ENABLE_SMART_DEDUP", True):
        return titles_data
    
    # 使用配置中的相似度阈值
    if similarity_threshold is None:
        similarity_threshold = CONFIG.get("DEDUP_CONFIG", {}).get("SIMILARITY_THRESHOLD", 0.8)
    ...
    for title, data in titles_data.items():
        ...
        sorted_titles.append((min_rank, title, data))
    
    sorted_titles.sort(key=lambda x: x[0])  # 按排名升序排序
    
    deduplicated = {}
    processed_normalized = []
    removed_count = 0
    
    for rank, title, data in sorted_titles:
        current_normalized = normalize_title_for_dedup(title)
        
        # 检查是否与已处理的标题相似
        is_similar = False
        for processed_norm in processed_normalized:
            similarity = calculate_title_similarity(title, processed_norm['original_title'])
            if similarity >= similarity_threshold:
                is_similar = True
                removed_count += 1
                print(f"智能去重: '{title}' 与 '{processed_norm['original_title']}' 相似度 {similarity:.2f}, 已跳过")
                break
        
        if not is_similar:
            deduplicated[title] = data
            processed_normalized.append({
                'normalized': current_normalized,
                'original_title': title
            })
    
    if removed_count > 0:
        print(f"智能去重完成: 原有 {len(titles_data)} 条，去重后 {len(deduplicated)} 条，移除 {removed_count} 条相似标题")
...
def process_source_data(
    ...
    """处理来源数据，合并重复标题，并应用智能去重"""
    # 先对当前批次的标题进行去重
    deduplicated_title_data = deduplicate_similar_titles(title_data)
    
    if source_id not in all_results:
        all_results[source_id] = deduplicated_title_data

        if source_id not in title_info:
            title_info[source_id] = {}

        for title, data in deduplicated_title_data.items():
        ...
        # 对新数据与已有数据进行跨批次去重
        combined_data = all_results[source_id].copy()
        combined_data.update(deduplicated_title_data)
        
        # 对合并后的数据再次去重
        final_deduplicated = deduplicate_similar_titles(combined_data)
        
        # 更新结果，只保留去重后的标题
        all_results[source_id] = final_deduplicated
        
        # 更新 title_info，只保留最终保留的标题
        new_title_info = {}
        for title, data in final_deduplicated.items():
            ranks = data.get("ranks", [])
            url = data.get("url", "")
            mobile_url = data.get("mobileUrl", "")
            
            if title in title_info.get(source_id, {}):
                # 已存在的标题，更新信息
                existing_info = title_info[source_id][title]
                existing_ranks = existing_info.get("ranks", [])
                ...
                new_title_info[title] = {
                    "first_time": existing_info.get("first_time", time_info),
                    "last_time": time_info,
                    "count": existing_info.get("count", 0) + 1,
                    "ranks": merged_ranks,
                    "url": existing_info.get("url", "") or url,
                    "mobileUrl": existing_info.get("mobileUrl", "") or mobile_url,
                }
            elif title in deduplicated_title_data:
                # 新标题
                new_title_info[title] = {
                    "first_time": time_info,
                    "last_time": time_info,
        ...
        if source_id not in title_info:
            title_info[source_id] = {}
        title_info[source_id] = new_title_info
...
def render_feishu_content(
    ...
    if CONFIG.get("DEDUP_CONFIG", {}).get("ENABLE_SMART_DEDUP", True):
        threshold = CONFIG.get("DEDUP_CONFIG", {}).get("SIMILARITY_THRESHOLD", 0.8)
        text_content += f"\n<font color='grey'>智能去重已启用 (相似度阈值: {threshold})</font>"

    if update_info:
        text_content += f"\n<font color='grey'>TrendRadaring 发现新版本 {update_info['remote_version']}，当前 {update_info['current_version']}</font>"
...
def render_dingtalk_content(
    ...
    if update_info:
        text_content += f"\n> TrendRadaring 发现新版本 **{update_info['remote_version']}**，当前 **{update_info['current_version']}**"
...
def send_to_dingtalk(
    ...
    payload = {
        "msgtype": "markdown",
        "markdown": {
            "title": f"TrendRadaring 热点分析报告 - {report_type}",
            "text": text_content,
        },
    }
